...
## Data Preparation & Filtering (`prep_data.py` & `split.py`)
...
3. **Advanced Variant Splitting (New):** A standard random split causes data leakage because identical multiple-choice variants for the same question can bleed across train and test sets. To fix this, we implemented a custom **Tiered Weighted Variant Split** (`split.py`):
   * Groups data strictly by `QuestionId` and `MC_Answer` (variants).
   * Greedily assigns entire variant groups to either the Train or Validation set.
   * Enforces strict class population minimums (`min_train=20`, `min_test=1`) across the splits to guarantee model stability. Doomed classes that fail this threshold are automatically dropped.
4. **CLI Toggle:** The pipeline supports toggling between the new robust `tiered` split (grouped by question variant) and the legacy `stratified` 95/5 random split via the `--split_method` CLI argument.
...
* `evaluate.py`: Generates predictions on the hold-out validation set and calculates final metrics.
...
**API call module (`llm_val.py`):**
This module helps orchestrate evaluating thousands of rows against remote APIs. In this project, the **api.groq.com** free tier is used.
